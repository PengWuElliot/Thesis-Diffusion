...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if not self.mask_data_root.exists():
            raise ValueError(mask_data_root)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.with_prior_preservation:
                    ...
                    # 计算逐像素的 MSE，不做 mean
                    pixelwise_loss = F.mse_loss(noise_pred.float(), target.float(), reduction="none")

                    # mask 已经是 [B, 1, H, W]，需要 broadcast 到 loss 的形状
                    # 设置 mask 区域权重较大（例如 2.0），非 mask 区域较小（例如 0.5）
                    mask_weight = mask * 2.0 + (1 - mask) * 0.5

                    # 应用权重
                    weighted_loss = pixelwise_loss * mask_weight
